...
def train(
    model, device, loader, optimizer, scheduler, num_classes, args, threshold, trainset=None
):
    """
...
def eval_gcn(model, device, loader, num_classes, args):
    ...
    if args.binary:
        ...
        nap, f = metrics_pharma.norm_ap_optimized(y_pred, y_true, num_classes = 8)
        auc = metrics_pharma.pltauc(y_pred, y_true, num_classes = 8)
...
def main():
    ...
    for epoch in range(init_epoch, args.epochs + 1):
        ...
        if args.advs:
            ...
            tr_loss = train(
                model, device, train_loader, optimizer, scheduler, args.nclasses, args, threshold
            )
...
if __name__ == "__main__":
    cls_criterion = torch.nn.BCELoss()
    #cls_criterion = torch.nn.CrossEntropyLoss() # Delete the softmax layer
